...
The architecture presented by web services has been widely accepted as a means of structuring the interactions between services as part of a distributed system and to work together to achieve a common goal . Currently , developers require greater standardization environments to provide additional interoperability between such services, but until mid-2004 any research group or group of experts had seriously considered the idea of ??proposing a standard for modeling communication between services web resources that have associated persistent . So in January of this year, several members of the organization \ emph { } Globus Alliance and IBM computer multinational defined with the help of experts from companies such as HP , SAP, Akamai, etc. . , Specification documents should occur in this model and the basis of an initial architecture . These documents were sent to the organization responsible for standardization , OASIS in March of 2004. Initially, two committees that take care of the study and development of certain parts of this new standard is formed. On the one hand , there was the \ emph { } WSRF Technical Committee which operated four specifications : \ emph { WS- ResourceProperties , WS- ResourceLifetime , WS- Servicegroup , and WS- BaseFaults } . Moreover, the \ emph { } WSN Technical Committee is responsible for the specifications : \ emph { WS- BaseNotification , WS- Topics , and WS- BrokeredNotification } . \ \

WS- Resource Framework is inspired by the work previously done by the \ emph { Global Grid Forum 's Open Grid Services Infrastructure ( OGSI ) Working Group } \ cite { Foster03 } . More specifically, it can be seen as a simple refactoring concepts and interfaces developed in the specification \ emph { OGSI V1.0 } such that exploits recent developments in the area of ??web services (eg , WS -Addressing ) . \ \

The aim of this paper is to introduce the basic concepts for the management and destruction of persistent web services , ie , web services that have associated resources to store the states of the same , as to the appearance of this approach , the web services were considered `` \ emph { stateless }'' and , therefore , they could temporarily store data or results of operations in a simple way for the user, since it was necessary to store them in a database outside the service data. In this approach, it is necessary to codify the relationship between the service and the application in terms of patterns using a series of widely studied technologies , for example, WS -Addressing and also will need to make their properties accessible from the outside through an interface . In this sense , we call \ emph { WS- Resource } to the association between a web service and a persistent resource .
...
WS- Resource Framework \ cite { BAN06 } is a specification developed by OASIS and some of the most pioneering IT companies , whose purpose is to define a generic framework for modeling and accessing resources associated with web services and the relationships between these resources in a Grid / Cloud environment. This approach consists of a set of specifications that define the representation of the WS- Resource in the terms that specify the messages exchanged and the XML documents related . It also includes mechanisms to describe the means to check the status of a resource and service description, which together form the definition of a WS- Resource. Also, define the steps needed to make the state of a web service accessible through its interface (described in WSDL) . \ \

Typically, web services interfaces provide users the ability to access and manipulate its condition , for example, data values ??that evolve by the interaction between various services. In other words, the message exchanges that are implemented in the behavior of the services are intended to allow persistent access to these resources. However, the notion of persistent resources underlying the implementation of the services is not as evident in the definition of the interface \ cite { Fost04 } . The messages sent and received these services involve ( or encourage the programmer to infer ) the existence of a type associated resource. It is therefore desirable that standards that allow the discovery , creation, insight , interaction and destruction of these resources are defined and the means chosen to accomplish this mission is as interoperable as possible. These observations have led to the appearance of the proposal discussed above , WS- Resource, to model states in the context of Web services. A WS- Resource is defined as the composition of a web service and its associated persistent resources , ie, \ emph { ( i ) } expressed as a combination of an XML document with a type defined by one or more \ emph { portTypes } ( a service may play a certain role if you implement all the \ emph { } comprising portTypes that role ) and \ emph { ( ii ) } addressed and accessed according to implied resource pattern , a derivation of the \ emph { Endpoint References WS -Addressing } . A \ emph { } Endpoint Reference shall consist of : Uniform Resource Identifier (URI ) parameters of the message that was sent to request that the \ emph { and } Endpoint Reference data on the interface used . In this exchange, the identifier of the persistent resource is encapsulated in a \ emph { and } Endpoint Reference used to identify the resource in any exchange of messages between services forming the choreography. So , WSRF to declare , accessing, monitoring and destroy WS -Resources by conventional techniques , which makes it easy to run as it is not necessary to complicate the decision logic of the resource owner to process service management messages . These conventional mechanisms comprise five technical specifications defining the means by which :
