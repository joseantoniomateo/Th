...
business applications can be developed, requiring less effor. These business applications
...
\item Define a set of web service orchestration concepts that are meant to be used by both external (abstract) and internal (executable) views of a business process. Such a business process defines the behavior of a single autonomous entity, typically operating in interaction with other similar entities. 
...
services from different companies interact jointly to perfom a particular task. In particular, it is a folklore 
...
In this section it is presented a summary of the main differences and sinergies between web services and
...
and define mechanisms to monitor its lifecycle. Surprisingly, the proces to create the WS-Resource
...
In conjuction with WSRF, Web Services Notification (WSN) specifications are focused 
...
since tey did not want to restrict the means by which
