...
known as ``utility'' services since customers are charged according to the consumption.  
This simple and old concept is revolutionising the way computational systems are evolving. 
Thus, major companies such as Google, Amazon or Microsoft are introducing in their business model
this new approach known as ``Utility Computing'' and that that was tried without success in the late 60s and early 70s. 
This new computing paradigm is often confused with Cloud and Grid Computing. Utility Computing 
is the underlying business model for a Grid or Cloud infrastructure, i.e. it
can be understood as a mean of charging customers  for computing services so that 
...
will be undertaken by the provider. As happens with the software , protocols or any computer-related paradigm , Cloud Computing must pass through a series of steps to check if all the publicity they are giving really serves companies to save costs and enhance the competitiveness or it's just one more way to increase income or, in the case of research , to obtain new funds. In this sense, some authors believe that Cloud Computing is nothing more than a new way of naming what life has been called Grid or Web Services and does not really involve any advancement in the field of computing. 

{\bf Añadir un parrafo sobre Grid y otro sobre Cloud}.

Cloud Computing , the old dream of providing computing services as a utility, has the potential to transform much of the computer industry , making the software more attractive to offer it as a service and shaping the way they are designed and buy the hardware. With this new approach, any entrepreneur with good ideas to offer services over the Internet will not need to invest heavily in equipment to carry out their project or need much initially hire staff to manage and maintain such equipment. Also, do not have to perform complicated previous studies to estimate the number of potential users and thus avoid some of the major headaches of project managers : the over- provisioning or under- provisioning. These two concepts together with the elasticity of resources can be considered as key to cloud computing , as the aim of reducing costs is directly proportional to the correct resource estimate in `` real time '' and this can only be correct estimate provided if the system meets the property of elasticity , ie , that you increase the range of relatively short time and you decrease the resources devoted to a task with a low economic cost . This approach can have , at first glance, the possibility of using formal methods with such systems is not seen , since normally formal techniques are used in the design of systems with time critical response and navigation systems of an aircraft, transportation of hazardous materials , etc. . Thus, it is not feasible to use cloud computing when very low response times are required, as, for example, a navigation system of an aircraft can not wait several minutes for new resources assigned to make a decision. However, if there are other systems where formal methods and can converge cloud computing , high availability systems .
