...
Nevertheless, scenarios are becoming more and more complex and designers do not need only a way to compose
a set of services, rather they also
need a way to compose these functionalities in the right order ? a way to define
business processes which will make use of the exposed functionalities and, they would
obviously prefer a relatively simple and straightforward way to define such processes. Thus, it is
a folklore that these environments are highly heterogeneous and, in particular, it is known
that business processes change relatively often. Therefore, designers and developers would like to modify them easily.
Sometimes, BPEL is thus compared to general purpose programming language, but it
is not as powerful. On the other hand, it is simpler and better suited for business
process definition and, therefore, BPEL cannot be considered a replacement but rather a supplement to
modern languages.
...
given. Executable processes allow us to specify the exact details of business processes.
...
and are not executable.Abstract processes serve a descriptive role, with more than one use case. One such use case
...
completed when mapping to an executable Process. As mentioned above it is possible to use WS-BPEL to define an executable business process, that is, 
...
As commented previously, BPEL is an XML-based language which
supports the web services technology stack, including SOAP, WSDL, UDDI, WS-Reliable
Messaging, WS-Addressing, WS-Coordination and WS-Transaction. A BPEL process specifies 
the exact order in which participating web services should be
